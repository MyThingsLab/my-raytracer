...
def _orthonormal_basis(normal: Array, backend: Backend) -> tuple[Array, Array]:
...
def sample_cosine_hemisphere(normal: Array, generator: Any, backend: Backend) -> Array:
    # Malley's method: a uniform disk sample lifted onto the hemisphere, giving
    # pdf(w) = cos(theta) / pi -- which cancels the cosine term in the render
    # equation, so the integrator's throughput update needs no explicit cosine.
    n = normal.shape[0]
    ...
    tangent, bitangent = _orthonormal_basis(normal, backend)
